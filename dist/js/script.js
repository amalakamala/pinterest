...
$(document).ready(function(){
	...
	for(var i = 0; i< descripcion.length/2;i++){
		...
		var url = descripcion[i].image_url;
		var title = descripcion[i].title;
		var des = descripcion[i].description;
		var user = descripcion[i].username; 
		var hastag = descripcion[i].hashtag;
		template +=	'<div class="contenedor">';
			template +=	'<img src="dist/img/'+ url +'" class="img-pint" id="img-'+ elId +'"  alt="">';
			template +=	`<div class="text-contenedor">`;
			template +=	`<div class="likes">`
			template +=	`<span><i class="fa fa-thumb-tack" aria-hidden="true"></i> 36,6k</span>`;
			template +=	`<span><i class="fa fa-check" aria-hidden="true"></i> 6</span>`;
			template +=	`</div>`;
			template += `<div class="titulo">`;
			...
			template += `<p class="name">`+ user +`</p>`;
			template += `<p>#`+ hastag +`</p>`;
			template += `</div>`;
			template += `</div>`;
		template += `</div>`;
	}
	$("#caja-json").append(template);
	$('.img-pint').click(function(){
		var id = $(this).attr("id");
